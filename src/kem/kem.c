...
OQS_API const char *OQS_KEM_alg_identifier(size_t i) {
	...
	const char *a[OQS_KEM_algs_length] = {
	    ...
	    OQS_KEM_alg_titanium_cca_std_kem, OQS_KEM_alg_titanium_cca_hi_kem, OQS_KEM_alg_titanium_cca_med_kem, OQS_KEM_alg_titanium_cca_super_kem, OQS_KEM_alg_ntru_hrss};
...
OQS_API OQS_KEM *OQS_KEM_new(const char *method_name) {
	if (0 == strcasecmp(method_name, OQS_KEM_alg_default)) {
		...
		return NULL;
#endif
	} else if (0 == strcasecmp(method_name, OQS_KEM_alg_ntru_hrss)) {
#ifdef OQS_ENABLE_KEM_ntru_hrss
		return OQS_KEM_ntru_hrss_new();
#else
		return NULL;
#endif
		// EDIT-WHEN-ADDING-KEM
